			...
			{	
				if (this.brewingItemStacks[var2] != null)
				{
					...
					if (ingredient.getItem() == Item.lightStoneDust)
					{
						...
						{
							brewings[0] = Brewing.thick;
						}
						else
						{
							for (int var3 = 0; var3 < list.tagCount(); var3++)
							{
								Brewing brewing = (list != null) ? Brewing.readFromNBT((NBTTagCompound)list.tagAt(var3)) : Brewing.awkward;
					...
					{
						for (Brewing brewing : TileEntityMixer.removeDuplicates(brewings))
						{
							if (brewing != null)
							{
